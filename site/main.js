...
window.abrirCategoria = (categoria) => {
    ...
    tituloSidebarEl.innerText = categoria;
    tituloSecaoEl.innerText = `Produtos em ${categoria}`;
    
    // Limpa a busca ao trocar de categoria
    barraPesquisa.value = "";
    
    // ALTERADO: Radio Button agora é text-black (Preto) em vez de text-green-600
...
window.filtrarPorSubcategoria = (categoria, subcategoria) => {
    ...
    tituloSecaoEl.innerText = `${subcategoria}`;
...
barraPesquisa.addEventListener("input", (e) => {
    ...
    contadorEl.innerText = `${visiveis} produtos encontrados`;
});

// --- FIREBASE ---
async function carregarProdutosFirebase(categoriaPrincipal, subcategoriaFiltro = null) {
    ...
    try {
        ...
        const catBusca = categoriaPrincipal; 
        
        const q = query(produtosRef, where("categorias", "array-contains", catBusca));
        ...
        listaProdutosEl.innerHTML = `<p class="col-span-full text-center text-red-500">Erro ao carregar produtos. Verifique se a categoria está correta no banco.</p>`;
    }
}

// --- FUNÇÃO AUXILIAR: LINK WHATSAPP ---
function gerarLinkZap(nomeProduto) {
    const texto = encodeURIComponent(`Olá! Tenho interesse no produto: *${nomeProduto}* e gostaria de mais informações.`);
        ...
        lista.forEach(produto => {
            ...
            const html = `
                <div class="bg-white rounded-lg shadow-md border-2 border-red-200 overflow-hidden relative group card-produto" data-nome="${produto.nome}">
                    ...
                    <div class="p-4">
                        <h3 class="font-bold text-gray-800 leading-tight mb-2 line-clamp-2">${produto.nome}</h3>
                        <div class="flex items-center justify-between mt-2">
                            <span class="text-xl font-bold text-red-600">${preco}</span>
                            <a href="${linkZap}" target="_blank" class="bg-red-600 text-white w-9 h-9 rounded-full flex items-center justify-center hover:bg-red-700 shadow-md transition">
...
function renderizarGrid(lista) {
    ...
    contadorEl.innerText = `${lista.length} produtos`;

    if (lista.length === 0) {
        listaProdutosEl.innerHTML = `<div class="col-span-full flex flex-col items-center justify-center py-10 bg-gray-50 rounded-lg"><i class="fas fa-box-open text-4xl text-gray-300 mb-3"></i><p class="text-gray-500">Nenhum produto encontrado.</p></div>`;
        return;
    }
    ...
    lista.forEach(produto => {
        ...
        listaProdutosEl.innerHTML += `
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition group card-produto" data-nome="${produto.nome}">
                ...
                <div class="p-4">
                    <p class="text-xs text-green-600 font-bold uppercase mb-1">${produto.subcategorias ? produto.subcategorias[0] : 'Item'}</p>
                    <h3 class="font-bold text-gray-800 leading-tight mb-2 line-clamp-2">${produto.nome}</h3>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if(navContainer) {
        document.getElementById('nav-scroll-left').addEventListener('click', () => navContainer.scrollBy({ left: -200, behavior: 'smooth' }));
        document.getElementById('nav-scroll-right').addEventListener('click', () => navContainer.scrollBy({ left: 200, behavior: 'smooth' }));
    }

    // Carrossel scroll
    const carouselContainer = document.getElementById('carousel-categorias');
    if(carouselContainer) {
        document.getElementById('carousel-scroll-left').addEventListener('click', () => carouselContainer.scrollBy({ left: -300, behavior: 'smooth' }));
        document.getElementById('carousel-scroll-right').addEventListener('click', () => carouselContainer.scrollBy({ left: 300, behavior: 'smooth' }));
    }
});